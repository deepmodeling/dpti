...
import yaml
from typing import TypeVar, Type, Union, Dict, Any, Optional
...
def parse_yaml_like_input(
    ...
    # Case 3: String or Path input - convert to string immediately
        ...
        if path_str.endswith(('.yaml', '.yml', '.json')):
            ...
            if not isinstance(raw_dict, dict):
                raise ValueError(f"YAML file does not contain a dictionary: {path_str=}")
        else:
            raw_dict = yaml.safe_load(path_str)
            if not isinstance(raw_dict, dict):
                raise ValueError(f"YAML content is not a dictionary")
    else:
    ...
    for field_name, field_value in raw_dict.items():
        if field_name in model_fields:
            ...
            if (hasattr(field_type, 'model_construct') and 
                isinstance(field_value, dict) and 
                getattr(field_type, '__origin__', None) != dict):  # 排除Dict类型
