...
# %%
from dpti.workflows.flows.base_flow import (
    BaseWorkflow,
    FlowMetaInfo,
    FlowProcedureControl,
    FlowRuntimeContext,
)
...
# %%
# %%
from dpti.workflows.simulations.equi_sim import (
    ExtractNVTForHTIConfLmp,
    NPTEquiSimulation,
    NPTEquiSimulationSettings,
    NPTResultToNVTConfLmp,
    NVTEquiSimulation,
    NVTEquiSimulationSettings,
)
from dpti.workflows.simulations.hti_sim import HTISimulation, HTISimulationSettings
from dpti.workflows.simulations.ti_sim import TISimulation, TISimulationSettings
...
class ThermoInputData(BaseModel, extra="ignore"):
    conf_lmp: str = Field(
        ..., validation_alias=AliasChoices("conf_lmp", "conf_file", "equi_conf")
    )
...
class FreeEnergyFlowWorkorder(BaseModel):  # implements WorkflowSettingsModel
    ...
    flow_trigger_dir_raw: str = Field(
        default="../../workflows/examples/Sn_beta_quicktest/"
    )
    flow_procedure_control: FlowProcedureControl = Field(default=FlowProcedureControl())
    domain_input_yaml_like: Union[Dict[str, Any], Path, str] = Field(
        default=load_default_yaml_like_asdict()
    )

    @computed_field
    @cached_property
    def domain_input_raw(
        self,
    ) -> FreeEnergyLineDomainInput:  # Partial[FreeEnergyLineDomainInput]:
        domain_input_raw = parse_yaml_like_input(
            self.domain_input_yaml_like, FreeEnergyLineDomainInput
        )
...
class FreeEnergyLineSettings(FlowSettingsBase):
    ...
    def flow_running_dir(self) -> str:
        flow_running_dir = os.path.join(
            self.flow_trigger_dir, self.flow_running_dirname
        )
    ...
    def nvt_simulation_settings(self):
        nvt_simulation_settings = NVTEquiSimulationSettings(
            **(
                ...
                | self.base_thermo_updates
                | {
                    "equi_conf": "npt_avg.lmp"
                }  # always use npt_avg.lmp for nvt simulation
            )
        )
    ...
    def hti_simulation_settings(self):
        ...
        nvt_if_dump_avg_posi = (
            self.domain_input_raw.nvt_simulation_settings.if_dump_avg_posi
        )
...
class FreeEnergyWorkflow(BaseWorkflow):
    ...
    def __init__(self, flow_workorder: FreeEnergyFlowWorkorder):
        ...
        flow_running_dir = os.path.join(
            flow_workorder.flow_trigger_dir, flow_workorder.flow_running_dirname
        )
    ...
    def init_workflow_services_injector(self, workflow_services_injector: Injector):
        ...
        self.workflow_services_injector.binder.bind(
            FlowRuntimeContext, to=self.flow_runtime_context
        )
    ...
    def _execute_impl(self) -> Any:
        ...
        with injection_context(injector):
            ...
            nvt_extracted_lmp = ExtractNVTForHTIConfLmp(
            ...
            ti_r = ti_sim(
                prev_results={
                    "free_energy_ref_value_point": hti_r["free_energy_ref_value_point"]
                }
            )
...
def free_energy_workflow_flow(flow_workorder: FreeEnergyFlowWorkorder):
    ...
    free_energy_workflow.init_workflow_services_injector(
        workflow_services_injector=my_injector
    )
