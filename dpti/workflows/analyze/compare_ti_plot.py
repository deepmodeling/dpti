...
#%%
def plot_free_energy_comparison(phase1_result: dict, phase2_result: dict, 
                              phase1_label: str = "Phase 1", 
                              phase2_label: str = "Phase 2") -> str:
    ...
    p1_tot_err = np.sqrt(np.array(phase1_result['data']['all_fe_stat_err'])**2 + 
    ...
    p2_tot_err = np.sqrt(np.array(phase2_result['data']['all_fe_stat_err'])**2 + 
                         np.array(phase2_result['data']['all_fe_inte_err'])**2)
    
    # Plot phase 1
    plt.errorbar(p1_temps, p1_fe, yerr=p1_tot_err, fmt='o-',
                label=phase1_label, color='blue', capsize=5,
                ecolor='lightblue', elinewidth=2)
    
    # Plot phase 2
    plt.errorbar(p2_temps, p2_fe, yerr=p2_tot_err, fmt='o-',
                label=phase2_label, color='red', capsize=5,
                ecolor='lightcoral', elinewidth=2)
    ...
    try:
        ...
        if len(cross_idx) > 0:
            ...
            plt.annotate(f'T = {t_cross:.1f}K\nG = {fe_cross:.6f}eV',
                        xy=(t_cross, fe_cross), xytext=(20, 20),
...
with open('../examples/Sn_beta_quicktest/TI_beta144xy_path_t_200K_50000bar_run3/TI_sim/new_job/result.json', 'r') as f:
    solid_result_info = json.load(f)

with open('../examples/Sn_beta_quicktest/TI_liquid128_path_t_1000K_50000bar_run0/TI_sim/new_job/result.json', 'r') as f:
...
def compare_multiple_runs(result_files: list, labels: list = None) -> str:
    ...
    for file_path in result_files:
        with open(file_path, 'r') as f:
            ...
            tot_err = np.sqrt(np.array(result['data']['all_fe_stat_err'])**2 + 
    ...
    ax2.fill_between(mean_temp, -std_fe, std_fe, 
...
result_files = glob.glob('../examples/Sn_beta_quicktest/TI_liquid128_path_t_1000K_50000bar_run*/TI_sim/new_job/result.json')


result_files.extend(glob.glob('../examples/Sn_beta_quicktest/TI_liquid128_path_t_1200K_50000bar_run*/TI_sim/new_job/result.json'))
result_files.extend(glob.glob('../examples/Sn_beta_quicktest/TI_liquid128_path_t_920K_50000bar_run*/TI_sim/new_job/result.json'))
result_files.extend(glob.glob('../examples/Sn_beta_quicktest/TI_liquid128_path_t_1080K_50000bar_run*/TI_sim/new_job/result.json'))
...
def plot_multiple_phase_comparison(
    ...
    for p1_result, p2_result in product(phase1_results, phase2_results):
        fe_diff = np.array(p2_result['data']['all_fe']) - np.array(p1_result['data']['all_fe'])
    ...
    plt.fill_between(temps, mean_diff-std_diff, mean_diff+std_diff, alpha=0.2, color='gray')
    plt.xlabel('Temperature (K)')
    ...
    plt.fill_between(temps, mean_diff-std_diff, mean_diff+std_diff, alpha=0.2, color='red')
    plt.xlabel('Temperature (K)')
    plt.ylabel('Free Energy Difference (eV)')
    plt.title(f'{phase2_label} - {phase1_label} Free Energy Difference\nDistribution')
    plt.grid(True)
    ...
    def find_crossing_temp(fe_diff, temps):
        ...
        try:
            # 在温度范围内寻找交点
            result = root_scalar(fe_interp, 
                               bracket=[temps[0], temps[-1]], 
                               method='brentq')
            return result.root
        except ValueError:
    ...
    crossing_temps_matrix = np.array(crossing_temps).reshape(len(phase2_results), len(phase1_results))
    ...
    sns.heatmap(crossing_temps_matrix,
                annot=True,  # 显示具体数值
                fmt='.1f',   # 数值格式保留一位小数
    ...
    plt.title(f'Phase Transition Temperatures (K)\nMean: {mean_temp:.1f}K ± {std_temp:.1f}K')
...
# 加载solid phase结果
for run in [0, 1, 2]:
    with open(f'../examples/Sn_beta_quicktest/TI_beta144xy_path_t_200K_50000bar_run{run}/TI_sim/new_job/result.json', 'r') as f:
        phase1_results.append(json.load(f))

for run in [0, 1, 2]:
    with open(f'../examples/Sn_beta_quicktest/TI_beta144xy_path_t_400K_50000bar_run{run}/TI_sim/new_job/result.json', 'r') as f:
        phase1_results.append(json.load(f))

for run in [0, 1, 2]:
    with open(f'../examples/Sn_beta_quicktest/TI_beta144xy_path_t_480K_50000bar_run{run}/TI_sim/new_job/result.json', 'r') as f:
        phase1_results.append(json.load(f))


# 加载liquid phase结果        
for run in [0, 2]:
    with open(f'../examples/Sn_beta_quicktest/TI_liquid128_path_t_1000K_50000bar_run{run}/TI_sim/new_job/result.json', 'r') as f:
        phase2_results.append(json.load(f))

for run in [0, 1]:
    with open(f'../examples/Sn_beta_quicktest/TI_liquid128_path_t_1200K_50000bar_run{run}/TI_sim/new_job/result.json', 'r') as f:
        phase2_results.append(json.load(f))

for run in [0, 1, 2]:
    with open(f'../examples/Sn_beta_quicktest/TI_liquid128_path_t_920K_50000bar_run{run}/TI_sim/new_job/result.json', 'r') as f:
        phase2_results.append(json.load(f))

for run in [0, 1, 2]:
    with open(f'../examples/Sn_beta_quicktest/TI_liquid128_path_t_1080K_50000bar_run{run}/TI_sim/new_job/result.json', 'r') as f:
        phase2_results.append(json.load(f))
