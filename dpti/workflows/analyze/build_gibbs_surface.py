...
class GibbsSurfaceBuilder:
    ...
    def load_result_file(self, file_path: str, phase_name: Optional[str] = None) -> Dict:
        """
        ...
        try:
            ...
            fe_stat_err = np.array(result_info['data'].get('all_fe_stat_err', [0] * len(temps)))
            fe_inte_err = np.array(result_info['data'].get('all_fe_inte_err', [0] * len(temps)))
    ...
    def load_multiple_files(self, file_pattern: str, phase_name: Optional[str] = None) -> List[Dict]:
        """
    ...
    def build_surface(self, temp_range: Optional[Tuple[float, float]] = None, 
    ...
    def plot_surface(self, T: np.ndarray, P: np.ndarray, fe_grid: np.ndarray, 
                    title: str = "Gibbs Free Energy Surface") -> str:
        """
        ...
        surf = ax.plot_surface(T, P, fe_grid, cmap=cm.viridis, 
        ...
        ax.scatter(data_points[:, 0], data_points[:, 1], data_points[:, 2], 
                  color='r', s=30, label='Data Points')
    ...
    def plot_contour(self, T: np.ndarray, P: np.ndarray, fe_grid: np.ndarray,
                    title: str = "Gibbs Free Energy Contour") -> str:
        """
        ...
        # Add contour lines
        contour_lines = plt.contour(T, P, fe_grid, 10, colors='white', alpha=0.5, linewidths=0.8)
        plt.clabel(contour_lines, inline=True, fontsize=8, fmt='%.4f')
        
        # Add the original data points
        data_points = np.array(self.data_points)
        plt.scatter(data_points[:, 0], data_points[:, 1], 
                   color='red', s=30, marker='o', label='Data Points')
    ...
    def find_phase_boundary(self, phase1: str, phase2: str, 
        ...
        if phase1 not in self.phase_data or phase2 not in self.phase_data:
            raise ValueError(f"Missing phase data. Available phases: {list(self.phase_data.keys())}")
        ...
        fe_grid1 = griddata(phase1_points, phase1_values, (T, P), method='cubic', fill_value=np.nan)
        fe_grid2 = griddata(phase2_points, phase2_values, (T, P), method='cubic', fill_value=np.nan)
        ...
        phase_boundary[np.abs(fe_diff) < 0.0001] = 1  # tolerance for numerical stability
        ...
        # Add data points
        plt.scatter(phase1_points[:, 0], phase1_points[:, 1], 
                   color='blue', s=30, marker='o', label=phase1)
        plt.scatter(phase2_points[:, 0], phase2_points[:, 1], 
                   color='red', s=30, marker='o', label=phase2)
    ...
    def plot_interpolated_surface(self, temp_range: Tuple[float, float] = None,
                                 press_range: Tuple[float, float] = None,
                                 resolution: int = 100,
                                 interpolation_method: str = 'cubic') -> str:
        """
        展示插值后的自由能曲面（包含原始数据点）
        ...
        ax.scatter(data_points[:, 0], data_points[:, 1], data_points[:, 2],
                  ...
                  label='Raw Data Points', zorder=10)
        ...
        ax.set_title(f'Interpolated Gibbs Surface ({interpolation_method.capitalize()} Interpolation)', pad=20)
    ...
    def save_data_points(self, output_path: str = None) -> str:
        """
        将加载的数据点保存为格式化文本文件
        ...
        data_array = data_array[np.lexsort((data_array[:,1], data_array[:,0]))]  # 按温度、压力排序
...
def main():
    ...
    # Generate plots
    surface_plot_base64 = builder.plot_surface(T, P, fe_grid, title="Gibbs Free Energy Surface")
    contour_plot_base64 = builder.plot_contour(T, P, fe_grid, title="Gibbs Free Energy Contour")
    ...
    try:
        ...
        print(f"Could not find phase boundary: {str(e)}")
...
if result_data:
    ...
    plt.scatter(result_data['temps'], result_data['fe'], c=result_data['press'], 
...
# 绘制3D散点图 - 使用自由能值来设置颜色
scatter = ax.scatter(temps, press, fe, c=fe, cmap='viridis', 
                    s=100, marker='o', edgecolors='k', alpha=0.8)
...
builder = GibbsSurfaceBuilder()
file_pattern = "../examples/Sn_beta_quicktest/TI_beta144xy_path*/TI_sim/new_job/result.json"

#%%
builder = GibbsSurfaceBuilder()
file_pattern = "../examples/Sn_beta_quicktest/TI_liquid128_path*/TI_sim/new_job/result.json"
...
for phase, color in phase_color_map.items():
    if phase in builder.phase_data:
        ...
        if phase_indices:
            ...
            # 绘制该相的散点
            ax.scatter(phase_temps, phase_press, phase_fe, 
                     c=color, s=80, label=phase, alpha=0.7, edgecolors='k')
