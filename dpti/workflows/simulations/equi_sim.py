...
class NPTEquiSimulation(
    SimulationBase[
        NPTEquiSimulationNodedata,  # NodedataType,
    ...
    def _prepare(self) -> Dict[str, Any]:
        with self.io_handler.jobdir_context(job_dirname=self.JOB_DIRNAME) as io_handler:
            ...
            lmp_str = equi.gen_equi_lammps_input(
                **lammps_input_kwargs_data.model_dump()
            )

            produced_file = io_handler.write_pure_file(
                file_path="in.lammps", file_content=lmp_str
            )
    ...
    def _run(self) -> str:
        ...
        # dlog.propagate = True
        submission_hash = self.workflow_services.job_executor.submit(
    ...
    def _extract(self) -> Dict[str, Any]:
        with self.io_handler.jobdir_context(job_dirname=self.JOB_DIRNAME) as io_handler:
            info_dict = equi.post_task(
                io_handler.job_dir, is_water=self.node_settings.if_water
            )
...
equi_summary_md_tmpl = """# thermodynamics           value                 err
...
class NVTEquiSimulation(
    SimulationBase[
        NVTEquiSimulationNodedata,  # NodeDataType,
    ...
    def _prepare(self) -> Dict[str, Any]:
        ...
        with self.workflow_services.io_handler.jobdir_context(
            ...
            if not lammps_input_kwargs_data.ens == "nvt":
                raise ValueError(
                    f"NVT simulation cannot be performed with ens={lammps_input_kwargs_data.ens} must be nvt"
                )
...
class NPTResultToNVTConfLmp:
    ...
    @workflow_task("run")
    def _run(self) -> str:
        with self.io_handler.jobdir_context(
            job_dirname=NPTEquiSimulation.JOB_DIRNAME
        ) as io_handler:
            npt_avg_conf_lmp = equi.npt_equi_conf(npt_dir=io_handler.job_dir)
            r_lmp = io_handler.write_pure_file(
                file_path="npt_avg.lmp", file_content=npt_avg_conf_lmp
            )
        return r_lmp
...
class ExtractNVTForHTIConfLmp:
    ...
    @workflow_task("run")
    def _run(self) -> str:
        nvt_job_dir = os.path.join(
            self.io_handler.flow_running_dir, NVTEquiSimulation.JOB_DIRNAME
        )
        hti_job_dir = os.path.join(
            self.io_handler.flow_running_dir, HTISimulation.JOB_DIRNAME
        )
