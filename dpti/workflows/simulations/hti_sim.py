...
from ... import hti, hti_ice, hti_liq, hti_water
...
class HTISimulationSettings(
    BaseModel,
    extra="allow",
):
    equi_conf: str
    ncopies: Optional[List[int]] = Field(
        default=[1, 1, 1], validation_alias=AliasChoices("ncopies", "copies")
    )
    lambda_lj_on: Optional[List[str]] = Field(
        ...,
        validation_alias=AliasChoices(
            "lambda_lj_on", "lambda_angle_on", "lambda_soft_on"
        ),
    )
    ...
    lambda_spring_off: List[str] = Field(
        ...,
        validation_alias=AliasChoices(
            "lambda_spring_off", "lambda_bond_angle_off", "lambda_soft_off"
        ),
    )  #
    protect_eps: float
    model: str
    ...
    thermo_freq: int = Field(
        ..., validation_alias=AliasChoices("thermo_freq", "stat_freq")
    )
    stat_skip: int
    stat_bsize: int
    temp: float
...
class HTISolidLammpsInput(BaseModel):
    ...
    dump_freq: int
    copies: Optional[List[int]] = Field(
        default=None, validation_alias=AliasChoices("copies", "ncopies")
    )  # deprecated
    crystal: str
...
class HTIResultData(TypedDict):
    ...
    pv_err: float
    free_energy_type: (
        str  # usually use Gibbs free energy for TI calculation convenience.
    )
...
class HTIIntegrationPath:
    ...
    def __repr__(self):
        r = (
            f"working for {self.template.field_name=}"
...
class HTIIntegrationPathTemplate:
    ...
    def __call__(self, seq_list: List[str], protect_eps: float = 1e-6):
        integration_path = HTIIntegrationPath(
            template=self, seq_list=seq_list, protect_eps=protect_eps
        )
...
class HTISimulation(
    SimulationBase[
        HTISimulationNodedata,  # NodedataType,
    ...
    def _initialize(self) -> None:
        self.hti_lammps_adapter = HTILammpsAdapter.create(
            node_settings=self.node_settings
        )
        self.integration_path_list = (
            self.hti_lammps_adapter.generate_integration_path_list(
    ...
    def _extract(self) -> HTIResultData:
        ...
        accurate_pv_value_from_npt = self.prev_results.get(
            "accurate_pv_value_from_npt", None
        )
        accurate_pv_err_value_from_npt = self.prev_results.get(
            "accurate_pv_err_value_from_npt", None
        )
        with self.workflow_services.io_handler.jobdir_context(
            job_dirname=self.JOB_DIRNAME
        ) as io_handler:
        ...
        info = HTIResultData(
            **extract_result, free_energy_ref_value_point=free_energy_ref_value_point
        )
        ...
        hti_result_fig_base64_md = get_hti_result_fig_base64_md(
            folder_list=[
                integration_path.subtasks_dirname
        ...
        self.io_handler.write_pure_file(
            file_path="hti_summary.md", file_content=hti_report_md
        )
    ...
    def _prepare_in_json_dict(self) -> Dict[str, Any]:
        ...
        calculated_info_dict = {
            ...
            "m_spring_k": [
                mass * self.node_settings.spring_k
    ...
    def _upload_required_files(self) -> None:
        ...
        self.io_handler.upload_file(
            file_path=self.conf_file, base_dir=self.io_handler.flow_running_dir
        )
    ...
    def _process_integration_path_list(
        self, integration_path_list: List[Any], in_json_dict: Dict[str, Any]
    ):
        for path_idx, integration_path in enumerate(integration_path_list):
            ...
            with self.workflow_services.io_handler.subjobdir_context(
                subjob_dirname=subtasks_dirname
            ) as io:  # enter the subtask director like `01.deep_on/`
                io.upload_file(
                    file_path="conf.lmp",
                    base_dir=os.path.join(
                        self.io_handler.flow_running_dir, self.JOB_DIRNAME
                    ),
                    #    new_file_name='conf.lmp')
                )
            ...
            for idx, lamb in enumerate(integration_path.all_lambda):
                ...
                lmp_str = self.hti_lammps_adapter.generate_lmp_str(
                    input_dict=hti_lammps_input_dict
                )
    ...
    def _write_subtasks_files(
        self, subtasks_dirname: str, subtask_name: str, lmp_str: str, lamb: float
    ):
        with self.workflow_services.io_handler.subjobdir_context(
...
class SolidHTI(HTILammpsAdapter):
    ...
    path_lj_on_tmpl = HTIIntegrationPathTemplate(
        field_name="lambda_lj_on", step_name="lj_on", subtasks_dirname="00.lj_on"
    )
    ...
    def generate_lmp_str(self, input_dict: Dict[str, Any]) -> str:
        ...
        lmp_str = hti._gen_lammps_input(**solid_iammps_input.model_dump())
        return lmp_str
    ...
    def generate_integration_path_list(self, switch: str) -> List[HTIIntegrationPath]:
        ...
        match switch:
            ...
            case "two-step":
                integration_path_list = [
                    self.path_deep_on_tmpl(seq_list=self.node_settings.lambda_deep_on),
                    self.path_spring_off_tmpl(
                        seq_list=self.node_settings.lambda_spring_off
                    ),
                ]
            case "three-step":
...
class IceHTI(HTILammpsAdapter):
    ...
    def generate_lmp_str(self, input_dict: Dict[str, Any]) -> str:
        ...
        lmp_str = hti._gen_lammps_input(**ice_iammps_input.model_dump())
        return lmp_str
...
class HTIWaterLammpsInput(BaseModel):
    step: str = Field(
        ..., description="HTI step", pattern="^(lj_on|deep_on|spring_off)$"
    )
    conf_file: str = Field(..., validation_alias=AliasChoices("conf_file", "equi_conf"))
    mass_map: List[float] = Field(
        ..., validation_alias=AliasChoices("mass_map", "model_mass_map")
...
class WaterHTI(HTILammpsAdapter):
    ...
    path_lj_on_tmpl = HTIIntegrationPathTemplate(
        field_name="lambda_lj_on", step_name="lj_on", subtasks_dirname="00.angle_on/"
    )
    ...
    def generate_lmp_str(self, input_dict: Dict[str, Any]) -> str:
        ...
        lmp_str = hti_water._gen_lammps_input(**water_iammps_input.model_dump())
        return lmp_str
...
class HTIIdealLiquidInput(BaseModel, extra="ignore"):
    step: str = Field(
        ..., description="Ideal HTI", pattern="^(soft_on|deep_on|soft_off)$"
    )
    conf_file: str = Field(..., validation_alias=AliasChoices("conf_file", "equi_conf"))
    mass_map: List[float] = Field(
        ..., validation_alias=AliasChoices("mass_map", "model_mass_map")
...
class LiquidHTI(HTILammpsAdapter):
    ...
    path_soft_on_tmpl = HTIIntegrationPathTemplate(
        field_name="lambda_soft_on", step_name="soft_on", subtasks_dirname="00.soft_on"
    )
    ...
    def generate_lmp_str(self, input_dict: Dict[str, Any]) -> str:
        ...
        lmp_str = hti_liq._gen_lammps_input_ideal(**ideal_iammps_input.model_dump())
        return lmp_str
        ...
        match switch:
            ...
            case "three-step":
                if self.node_settings.lambda_lj_on is None:
                    raise ValueError("lambda_lj_on is not set")
                integration_path_list = [
                    self.path_soft_on_tmpl(
                        seq_list=self.node_settings.lambda_lj_on
                    ),  # note lambda_lj_on is used for soft_on for compatibility Solid reason
                    self.path_deep_on_tmpl(seq_list=self.node_settings.lambda_deep_on),
                    self.path_soft_off_tmpl(
                        seq_list=self.node_settings.lambda_spring_off
                    ),  # note lambda_lj_off is used for soft_off for compatibility Solid reason
                ]
            case _:
                raise ValueError(f"Error option {switch=}")
        return integration_path_list
...
def plot_hti_results(folder_list: List[str], base_path: str) -> str:
    ...
    for idx, folder in enumerate(folder_list):
        ...
        diff_e, stat_err, sys_integ_err = integrate_range_hti(
            lmbda, U_integrand, U_integrand_err, scheme="simpson"
        )

        axes[idx].errorbar(
            lmbda,
            U_integrand,
            yerr=U_integrand_err,
            fmt="o-",
            label=folder,
            color="blue",
            capsize=5,
        ...
        axes[idx].set_ylim(
            np.min(U_integrand) - y_range * 0.1, np.max(U_integrand) + y_range * 0.1
        )
...
def get_hti_result_fig_base64_md(folder_list: List[str], base_path: str) -> str:
    ...
    img_base64 = plot_hti_results(folder_list=folder_list, base_path=base_path)
