...
import asyncio
from typing import List, Dict, TypeVar, TypedDict, Union, Any, Type, NamedTuple, Optional, ClassVar
...
class TISimulationFreeEnergyData(ResultDataBase):
    ...
    all_press: List[float] = Field(..., description="Pressure points")
    all_fe: List[float] = Field(..., description="Free energy values")
    all_fe_stat_err: List[float] = Field(..., description="Statistical errors in free energy")
    all_fe_inte_err: List[float] = Field(..., description="Integration errors in free energy")
...
class TISimulationResult(ResultDataBase):
    ...
    start_point_info: ThermodynamicPoint = Field(..., description="Initial state point data")
    end_point_info: ThermodynamicPoint = Field(..., description="Final state point data")
    ti_free_energy_data: TISimulationFreeEnergyData = Field(..., description="Thermodynamic data across all states")
...
class TISimulation(
    SimulationBase[TISimulationNodeData,  # NodeDataType, 
                   ...
                   TISimulationResultData] # ReturnType
                   ):
    ...
    def _prepare(self) -> Dict[str, Any]:
        with self.workflow_services.io_handler.jobdir_context(job_dirname=self.JOB_DIRNAME) as io_handler:
            self.upload_predefined_files(upload_local_files_fields=self.UPLOAD_LOCAL_FILES_FIELDS)
            ...
            thermo_path_seq = getattr(self.node_settings, self.ti_integration_path.path_field_name)
    ...
    def _extract(self) -> TISimulationResultData:
        ...
        with self.workflow_services.io_handler.jobdir_context(job_dirname=self.JOB_DIRNAME) as io_handler:
            ...
            Eo_err = self.prev_results['free_energy_ref_value_point']['gibbs_free_energy_err']
            if self.node_settings.path == 't':
                To = self.node_settings.temp
            elif self.node_settings.path == 'p':
                To = self.node_settings.pres
            else:
                raise RuntimeError(f"Known To value {self.node_settings.path=} {self.node_settings=}")
            ...
            io_handler.write_pure_file(file_path='ti_summary.md', file_content=ti_summary_md)
        ...
        with self.workflow_services.io_handler.jobdir_context(job_dirname=self.JOB_DIRNAME) as io_handler:
            ...
            io_handler.write_pure_file(file_path='ti_simulation_result.json',
...
class TILammpsInput(BaseModel):
    ...
    thermo_freq: int
    dump_freq: int
    copies: List[int] = Field([1,1,1], validation_alias=AliasChoices('copies', 'ncopies'))
    if_meam: bool
    meam_model: Optional[Dict[str, Any]] = None
...
def get_ti_summary_md(result_info:Dict[str, Any]) -> str:
    ...
    for temp, press, fe, fe_stat_err, fe_inte_err, fe_tot_err in zip(
        ...
        rows.append(f"{temp:9.2f}  {press:15.8e}  {fe:20.12f}  {fe_stat_err:9.2e}  {fe_inte_err:9.2e}  {fe_tot_err:9.2e}")
    
    result_fig_base64_md_str = get_ti_result_fig_base64_md_str(result_info=result_info)

    extra_info = "extra info:\n {result_info}".format(result_info=result_info)
...
def plot_ti_result(result_info: dict) -> str:
    ...
    fig_title_tmpl = "Free Energy vs Temperature with Error Analysis.\n job_dir={job_dir}"
    try:
