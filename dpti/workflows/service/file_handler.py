...
current_io_context: ContextVar[Optional["IOHandler"]] = ContextVar(
    "current_io_context", default=None
)
...
class IOHandler(Protocol):
    ...
    def upload_file(
        self, file_path: str, base_dir: str, new_file_name=None
    ) -> str:  # os.path.join(base_dir, file_path)
        raise NotImplementedError
...
class LocalFileHandler:  # implement IOHandler
    def __init__(
        self, flow_trigger_dir: str = "./", flow_running_dirname: str = "./"
    ) -> None:
        self.flow_trigger_dir = flow_trigger_dir
        self.flow_running_dirname = flow_running_dirname
        self.flow_running_dir = os.path.join(
    ...
    def setup_flow_running_dir(self) -> str:
        if os.path.isdir(self.flow_running_dir):
            ...
            produced_flow_running_dir = self.create_path_with_backup(
                self.flow_running_dir
            )
    ...
    def jobdir_context(self, job_dirname: str) -> Iterator[Any]:
        ...
        try:
            ...
            print(
                f"LocalFileHandler: Entering job context {self.flow_running_dir=} {job_dirname=} {self.job_dir=}"
            )
    ...
    def subjobdir_context(self, subjob_dirname: str = "./") -> Iterator[Any]:
        ...
        try:
            ...
            print(
                f"LocalFileHandler: Entering subjob_dir context {ori_job_dir=} {subjob_dirname=} {self.job_dir=}"
            )
            # Ensure the directory exists
            if not os.path.isdir(self.job_dir):
                self.job_dir = self.create_path_with_backup(self.job_dir)
    ...
    def create_relative_symlink_file(
        self, source_base_dir, source_file_path, target_dir, new_linkfile_name=None
    ):
        abs_file_path = os.path.join(source_base_dir, source_file_path)
        if not os.path.isfile(abs_file_path):
            raise RuntimeError(
                f"{os.getcwd()=}, {abs_file_path=} must be a file. "
        ...
        rel_target_linkfile_path = os.path.relpath(
            target_linkfile_path, start=self.flow_running_dir
        )
