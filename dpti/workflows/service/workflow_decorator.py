...
class PrefectTaskDecorator:
    ...
    def __call__(self, obj=None, **decorator_task_settings):
        ...
        if obj is None:
            ...
            def configured_decorator(inner_obj):
                combined_task_settings = {**self.global_task_settings, **decorator_task_settings}
    ...
    def _decorate_class(self, cls, combined_task_settings):
        ...
        for method_name, original_method in original_methods.items():
            ...
            def create_wrapped_method(method_name, original_method, final_method_settings):
                @task(**final_method_settings)
                @functools.wraps(original_method)
                def wrapped_method(self, *args, **kwargs):
                    print(f"Running {method_name} with settings: {final_method_settings}")
                    return original_method(self, *args, **kwargs)
                return wrapped_method
            
            # Replace original method
            setattr(cls, method_name, create_wrapped_method(
...
@workflow_task_decorator(task_configs={
