...
def compute_bonds(box, atype, posis, max_roh=1.3, uniq_hbond=True):
    natoms = len(posis)
    bonds = []
...
def add_bonds(lines_, max_roh=1.3):
    ...
    for ii in range(len(bonds)):
        if atype[ii] == 1:
            assert len(bonds[ii]) == 2, "ill defined O atom %d has H %s" % (
                ii,
                bonds[ii],
            )
        elif atype[ii] == 2:
            assert len(bonds[ii]) == 1, "ill defined H atom %d has O %s" % (
                ii,
                bonds[ii],
            )
...
def min_oho(box, atype, posis):
    ...
    for ii in range(natoms):
        ...
        all_dist.sort()
        doo = np.linalg.norm(
            posi_diff(box, posis[all_dist[0][1]], posis[all_dist[1][1]])
        )
