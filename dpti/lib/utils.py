...
def _parse_one_str(in_s):
    ...
    if len(fmt_s) == 1:
        ...
        return np.arange(
            float(fmt_s[0]), float(fmt_s[1]) - float_protect, float(fmt_s[2])
        )
...
def parse_seq(in_s, *, protect_eps=None):
    ...
    if isinstance(in_s, list) and isinstance(in_s[0], str):
        ...
        raise RuntimeError(
            "the type of seq should be one of: string, list_of_strings, list_of_floats"
        )
...
def integrate_simpson(xx, yy, ye):
    if len(xx) % 2 == 0:
        ...
        for ii in range(0, len(xx) - 2, 2):
            two_h = xx[ii + 2] - xx[ii]
            diff_e += 1.0 / 6.0 * two_h * (yy[ii] + 4.0 * yy[ii + 1] + yy[ii + 2])
            err += np.square(1.0 / 6.0 * two_h * 1.0 * ye[ii])
            err += np.square(1.0 / 6.0 * two_h * 4.0 * ye[ii + 1])
            err += np.square(1.0 / 6.0 * two_h * 1.0 * ye[ii + 2])
        return diff_e, np.sqrt(err)
...
def integrate_simpson_nonuniform(x, f, fe):
    ...
    for i in range(1, N, 2):
        hph = h[i] + h[i - 1]
        pref1 = (h[i] ** 3 + h[i - 1] ** 3 + 3.0 * h[i] * h[i - 1] * hph) / (
            6 * h[i] * h[i - 1]
        )
        pref0 = (2.0 * h[i - 1] ** 3 - h[i] ** 3 + 3.0 * h[i] * h[i - 1] ** 2) / (
            6 * h[i - 1] * hph
        )
        pref2 = (2.0 * h[i] ** 3 - h[i - 1] ** 3 + 3.0 * h[i - 1] * h[i] ** 2) / (
            6 * h[i] * hph
        )
        result += f[i] * pref1
        result += f[i - 1] * pref0
    ...
    if (N + 1) % 2 == 0:
        pref1 = (2 * h[N - 1] ** 2 + 3.0 * h[N - 2] * h[N - 1]) / (
            6 * (h[N - 2] + h[N - 1])
        )
...
def integrate_sys_err_trapezoidal(xx, yy):
    ...
    for ii in range(1, len(xx) - 2):
        err += interval_sys_err_trapezoidal(
            xx[ii - 1 : ii + 3], yy[ii - 1 : ii + 3], "middle"
        )
...
def integrate_range_trapezoidal(xx, yy, ye):
    ...
    for ii in range(1, nn):
        inter_i, inter_se = integrate_trapezoidal(
            xx[ii - 1 : ii + 1], yy[ii - 1 : ii + 1], ye[ii - 1 : ii + 1]
        )
    ...
    for ii in range(1, nn - 2):
        inte_err[ii + 1] = inte_err[ii] + interval_sys_err_trapezoidal(
            xx[ii - 1 : ii + 3], yy[ii - 1 : ii + 3], "middle"
        )
    inte_err[nn - 1] = inte_err[nn - 2] + interval_sys_err_trapezoidal(
        xx[-3:], yy[-3:], "right"
    )
    return xx, inte, inte_err, stat_err
...
def _integrate_range_simpson_inner(xx, yy, ye):
    ...
    for ii in range(2, nn, 2):
        inter_i, inter_se = integrate_simpson_nonuniform(
            xx[ii - 2 : ii + 1], yy[ii - 2 : ii + 1], ye[ii - 2 : ii + 1]
        )
        new_xx.append(xx[ii])
...
def compute_nrefine(all_t, integrand, err, error_scale=None):
    ...
    interval_err = []
    interval_err.append(
        interval_sys_err_trapezoidal(all_t[0:3], integrand[0:3], "left")
    )
    for ii in range(1, ntask - 2):
        interval_err.append(
            interval_sys_err_trapezoidal(
                all_t[ii - 1 : ii + 3], integrand[ii - 1 : ii + 3], "middle"
            )
        )
    interval_err.append(
        interval_sys_err_trapezoidal(all_t[-3:], integrand[-3:], "right")
    )
    ...
    for ii in range(ntask - 1):
        ...
        interval_nrefine.append(
            max(1, int(np.ceil(np.sqrt(interval_err[ii] / err_dist))))
        )
...
def integrate_range_hti(all_lambda, de, de_err, scheme="s"):
    new_lambda, i, i_e, s_e = integrate_range(all_lambda, de, de_err, scheme=scheme)
    # print('debug:range_hti', new_lambda[-1], all_lambda[-1])
    if new_lambda[-1] != all_lambda[-1]:
        if new_lambda[-1] == all_lambda[-2]:
            _, i1, i_e1, s_e1 = integrate_range(
                all_lambda[-2:], de[-2:], de_err[-2:], scheme="t"
            )
            diff_e = i[-1] + i1[-1]
...
if __name__ == "__main__":
    ninter = 20
    error = 1e-1
    xx = np.arange(0, 1 + 1e-10, 1.0 / ninter)
    yy = np.exp(xx)
    ye = np.ones(xx.shape) * error / np.sqrt(12)
    rng = np.random.default_rng()
    yy = yy + (rng.random([ninter + 1]) * error - 0.5 * error)
