...
class SSHSession:
    def __init__(self, jdata):
        ...
        self.ssh = self._setup_ssh(
            self.remote_host, self.remote_port, username=self.remote_uname
        )

    def _setup_ssh(self, hostname, port, username=None, password=None):
...
class RemoteJob:
    def __init__(self, ssh_session, local_root):
    ...
    def upload(self, job_dirs, local_up_files, dereference=True):
        cwd = os.getcwd()
    ...
    def block_checkcall(self, cmd):
        ...
        if exit_status != 0:
            raise RuntimeError(
                "Get error code %d in calling through ssh with job: %s ",
                (exit_status, self.job_uuid),
            )
