...
def gen_equi_lammps_input(
    ...
    equi_lammps_input = (
        equi_header
        + equi_force_field
        + equi_thermo_settings
        + equi_dump_settings
        + equi_ensemble_settings
    )
...
def add_module_subparsers(main_subparsers):
    ...
    module_subparsers = module_parser.add_subparsers(
        help="commands for equilibration simulations", dest="command", required=True
    )
...
def add_subparsers(module_subparsers):
    ...
    parser_stat = module_subparsers.add_parser(
        "stat-bond", help="Statistic of the bonds"
    )
    ...
    parser_compute = module_subparsers.add_parser(
        "compute", help="Compute thermodynamics"
    )
...
def _main():
    parser = argparse.ArgumentParser(description="equilibration simulations")
