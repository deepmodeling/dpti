...
def _ff_angle_on(lamb, model, bparam, sparam):
...
def _post_tasks(iter_name, step, natoms, scheme="s"):
    ...
    if step == "angle_on":
        ...
        all_err = np.sqrt(np.square(all_ag_e / all_lambda) + np.square(all_dp_e))
    elif step == "deep_on":
        de = all_dp_a
        all_err = all_dp_e
    elif step == "bond_angle_off":
        ...
        all_err = np.sqrt(
            np.square(all_bd_e / (1 - all_lambda))
    ...
    if new_lambda[-1] != all_lambda[-1]:
        if new_lambda[-1] == all_lambda[-2]:
            _, i1, i_e1, s_e1 = integrate_range(
                all_lambda[-2:], de[-2:], all_err[-2:], scheme="t"
            )
            diff_e = i[-1] + i1[-1]
...
def spring_inte(temp, kk, r0):
    kto2k = (
        pc.Boltzmann
        * temp
        / (2.0 * kk * pc.electron_volt / (pc.angstrom * pc.angstrom))
    )
...
def compute_ideal_mol(iter_name):
    ...
    # N!
    fe += natoms_o * np.log(natoms_o) - natoms_o + 0.5 * np.log(2.0 * np.pi * natoms_o)
    fe += natoms_h * np.log(np.sqrt(2))
    # plus PV
    if "npt" in ens:
        fe += natoms_o + 5.0 / 6.0 * natoms_h
    # to kbT log Z
...
def post_tasks(iter_name, natoms, method="inte", scheme="s"):
    ...
    if method == "inte":
        e2, err2, tinfo2 = _post_tasks(
            subtask_name, "bond_angle_off", natoms, scheme=scheme
        )
    elif method == "mbar":
...
def add_module_subparsers(main_subparsers):
    module_parser = main_subparsers.add_parser(
        "hti_water", help="Hamiltonian thermodynamic integration for liquid water"
    )
    module_subparsers = module_parser.add_subparsers(
        help="commands of Hamiltonian thermodynamic integration for liquid water",
        dest="command",
        required=True,
    )
...
def handle_compute(args):
    ...
    with open(os.path.join(args.JOB, "in.json")) as j:
        jdata = json.load(j)
