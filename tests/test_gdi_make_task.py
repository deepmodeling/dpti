...
class TestGdiMakeTask(unittest.TestCase):
    ...
    def test_deepmd_uses_phase_graph_name(self, patch_random):
        ...
        with open(os.path.join(test_dir, "in.lammps")) as fp:
            lmp_input = fp.read()
        self.assertIn("pair_style      deepmd graph.0.pb", lmp_input)
        self.assertNotIn("pair_style      deepmd graph.pb", lmp_input)

    def test_setup_dpdt_phase_specific_model_suffixes(self):
        parent_dir = os.path.join(self.test_dir, "phase_model_suffixes")
        task_dir = os.path.join(parent_dir, "gdi_job")
        os.makedirs(parent_dir)
        shutil.copyfile("conf.lmp", os.path.join(parent_dir, "conf.0.lmp"))
        shutil.copyfile("alpha.lmp", os.path.join(parent_dir, "conf.1.lmp"))
        shutil.copyfile("graph.pb", os.path.join(parent_dir, "solid_model.pth"))
        shutil.copyfile("beta.lmp", os.path.join(parent_dir, "liquid_model.pb"))
        jdata = {
            "phase_i": {
                "name": "PHASE_0",
                "equi_conf": "conf.0.lmp",
                "model": "solid_model.pth",
            },
            "phase_ii": {
                "name": "PHASE_1",
                "equi_conf": "conf.1.lmp",
                "model": "liquid_model.pb",
            },
            "mass_map": [118.71],
            "nsteps": 5000,
            "timestep": 0.002,
        ...
        self.assertTrue(os.path.isfile(os.path.join(task_dir, "graph.0.pth")))
