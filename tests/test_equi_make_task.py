...
class TestEquiMakeTask(unittest.TestCase):
    ...
    def test_nvt_use_npt_avg(self, patch_random):
        ...
        check_file_list = ["in.lammps", "npt_avg.lmp", "graph.pb"]
        for file in check_file_list:
            f1 = os.path.join(benchmark_dir, file)
            f2 = os.path.join(test_dir, file)
            self.assertEqual(get_file_md5(f1), get_file_md5(f2), msg=(f1, f2))
    ...
    def test_run_task_model_file_from_settings(self):
        ...
        with open(os.path.join(test_dir, "equi_settings.json"), "w") as fp:
            json.dump({"model": "/tmp/model.pth"}, fp)

        self.assertEqual(dpti.equi._get_task_model_file(test_dir), "model.pth")
