...
class TestGetThermo(unittest.TestCase):
    def setUp(self):
        ...
        self.log_file = os.path.join("lammps_test_files", "get_thermo.log.lammps")
        self.data_file = os.path.join("lammps_test_files", "get_thermo.data")
        # dump fmt=%.12e
...
class TestGetLastDump(unittest.TestCase):
    def setUp(self):
        ...
        self.last_dump_file = os.path.join(
            "lammps_test_files", "test_get_last_dump.dump.equi.lastdump"
        )
        self.dump_file = os.path.join(
            "lammps_test_files", "test_get_last_dump.dump.equi"
        )

    def test_normal(self):
        with open(self.last_dump_file) as f:
