...
class TestLocalFileHandler(unittest.TestCase):
    ...
    def test_write_pure_file(self):
        ...
        rel_file_path = self.file_handler.write_pure_file(file_path, file_content)
        # 构建完整路径用于验证文件内容
...
>>>>>>> b644dedbcc6d0cb76d7cb250440013f6122c4228
    ...
    def test_create_relative_symlink_file(self):
        ...
        self.assertTrue(os.path.islink(abs_linkfile_path))
        self.assertEqual(os.path.basename(abs_linkfile_path), "test_file.txt")
...
<<<<<<< HEAD
        ...
        abs_symlink_path = os.path.join(self.flow_running_dir, rel_symlinks[0])
        self.assertTrue(os.path.islink(abs_symlink_path))
        self.assertEqual(os.path.basename(abs_symlink_path), "test_file.txt")

        link_target = os.readlink(abs_symlink_path)
        abs_target = os.path.join(os.path.dirname(abs_symlink_path), link_target)
        self.assertTrue(os.path.samefile(abs_target, test_file_path))
...
>>>>>>> b644dedbcc6d0cb76d7cb250440013f6122c4228
    ...
    #         # test writing file in subdir
    #         self.file_handler.write_pure_file("test_file.txt", "Hello from subdir")
    #         self.assertTrue(os.path.exists(os.path.join(self.file_handler.job_dir, "test_file.txt")))
