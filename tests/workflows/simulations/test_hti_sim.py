...
from dpti.workflows.simulations.hti_sim import (
    HTIIntegrationPathTemplate,
    HTISimulation,
    HTISimulationSettings,
    SolidHTI,
)
...
class TestHTISimulation:
    ...
    def test_prepare_integration_paths(self):
        ...
        assert len(paths) == 3
        assert [p.template.step_name for p in paths] == [
            "lj_on",
            "deep_on",
            "spring_off",
        ]
    ...
    def test_prepare_file_handling(self, mock_file):
        ...
        in_json_call = next(c for c in calls if c[1]["file_path"] == "in.json")
