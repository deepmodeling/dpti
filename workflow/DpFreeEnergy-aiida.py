...
from dpdispatcher.submission import Machine, Resources, Submission, Task
...
def HTI_sim(job_work_dir):
    ...
    task_dir_list = [
        os.path.relpath(ii, start=job_work_dir.value) for ii in task_abs_dir_list
    ]
...
@calcfunction
def HTI_end(job_work_dir, start_info, NPT_end_info):
    if_liquid = start_info["if_liquid"]
...
def TI_end(job_work_dir, start_info, HTI_end_info):
    ...
    if os.path.isfile(result_file_path):
        ...
        info = ti.compute_task(
            job_work_dir.value, inte_method="inte", Eo=Eo, Eo_err=Eo_err, To=To
        )
    print(info)
    return Dict(dict=info)
...
def TI_workflow(dag_run):
    ...
    hti_job_dir = HTI_sim(HTI_start(start_info=start_info, NVT_end_info=NVT_end_info))
    HTI_end_info = HTI_end(hti_job_dir, start_info, NPT_end_info)

    ti_end_info = TI_sim(TI_start(start_info=start_info, HTI_end_info=HTI_end_info))
