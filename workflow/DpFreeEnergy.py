...
def all_start_check():
    ...
    dag_work_dirname = (
        str(target_temp) + "K-" + str(target_pres) + "bar-" + str(conf_lmp)
    )
...
def NPT_sim(job_work_dir):
    ...
    )  # Files to transfer forward and backwards
    context = get_current_context()
...
def HTI_sim(job_work_dir):
    ...
    task_list = [
        Task(
            ...
            task_work_path=ii,
            forward_files=_forward_files_with_model(
                job_work_dir, "in.json", ["in.lammps", "*lmp"]
            ),
            backward_files=["log.lammps"],
        )
...
def TI_end(job_work_dir, start_info, HTI_end_info):
    ...
    if os.path.isfile(result_file_path):
        ...
        info = ti.compute_task(
            job_work_dir, inte_method="inte", Eo=Eo, Eo_err=Eo_err, To=To
        )
    print(info)
    return info


default_args = {"owner": "fengbo", "start_date": datetime(2021, 1, 1, 8, 00)}
